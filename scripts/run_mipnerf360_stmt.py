...
def train_scene(gpu, scene, factor):
    cmd = f"OMP_NUM_THREADS=4 python train.py -s 360_v2/{scene} -m {output_dir}/{scene} --eval -r {factor} --port {6009+int(gpu)} --kernel_size 0.1"
    print(cmd)
    if not dry_run:
        os.system(cmd)
    
    for scale in [8, 4, 2, 1]:
        cmd = f"OMP_NUM_THREADS=4 python render.py -m {output_dir}/{scene} -r {scale} --data_device cpu --skip_train"
        print(cmd)
        if not dry_run:
            os.system(cmd)
...
def worker(gpu, scene, factor):
    ...
    print(f"Finished job on GPU {gpu} with scene {scene} time cost: {(end-start)/60}mins\n")
