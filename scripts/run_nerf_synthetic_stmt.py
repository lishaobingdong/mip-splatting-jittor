...
def train_scene(gpu, scene, factor):
    cmd = f" python train.py -s {dataset_dir}/{scene} -m {output_dir}/{scene} --eval --white_background --kernel_size 0.1"
    print(cmd)
    if not dry_run:
        os.system(cmd)

    cmd = f" python render.py -m {output_dir}/{scene} --skip_train"
    print(cmd)
    if not dry_run:
        os.system(cmd)
        
    cmd = f" python metrics.py -m {output_dir}/{scene}"
    print(cmd)
    if not dry_run:
        os.system(cmd)
